...
class DataLoader_video_test_segment(Sequence):
    def __init__(self, split_path, frames_path, index, granularity, batch_size = 4):
        self.batch_size = batch_size
        self.path = frames_path
    ...
    def _get_video(self, vid_name):
        images_all = sorted(glob.glob(self.path + vid_name + "/*"))
        images = images_all[ (self.index * len(images_all)) // self.granularity : ((self.index+1) * len(images_all)) // self.granularity]
        files = []
